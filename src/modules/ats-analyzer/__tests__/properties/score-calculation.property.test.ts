...
describe('ATS Score Calculator - Property Tests', () => {
	...
	it('Property 1: Hard skills have highest impact on score', () => {
		...
		fc.assert(
			fc.property(
				...
				fc.array(hardSkillKeywordArbitrary, { maxLength: 10, minLength: 5 }),
				fc.array(softSkillKeywordArbitrary, { maxLength: 10, minLength: 5 }),
	...
	it('Property 1: Empty job keywords results in score of 100', () => {
		// **Feature: optimization-cockpit, Property 1: Score Calculation Weighting**
		fc.assert(
			fc.property(resumeContentArbitrary, (resume) => {
	...
	it('Property 1: Score increases when more keywords match', () => {
		...
		fc.assert(
			fc.property(
				fc.array(hardSkillKeywordArbitrary, { maxLength: 5, minLength: 2 }),
				(hardSkills) => {
...
function createResumeWithSkills(skills: string[]) {
	return {
		...
		education: [],
		languages: [],
		meta: { completionScore: 0, template: 'default' },
		skills: {
