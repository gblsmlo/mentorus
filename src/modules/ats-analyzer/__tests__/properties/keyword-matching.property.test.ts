...
import { type CategorizedKeyword, calculateATSScore } from '../../utils/score-calculator'
import {
	GENERAL_KEYWORDS,
	HARD_SKILLS,
	SOFT_SKILLS,
...
describe('Keyword Matching and Categorization - Property Tests', () => {
	...
	it('Property 2: All extracted keywords have valid categories', () => {
		// **Feature: optimization-cockpit, Property 2: Keyword Matching and Categorization**
		const jobDescriptionArbitrary = fc
			.array(fc.constantFrom(...HARD_SKILLS, ...SOFT_SKILLS, ...GENERAL_KEYWORDS), {
				maxLength: 15,
				minLength: 3,
			})
		...
		fc.assert(
			fc.property(jobDescriptionArbitrary, (jobDescription) => {
	...
	it('Property 2: Known hard skills are categorized as hard_skill', () => {
		// **Feature: optimization-cockpit, Property 2: Keyword Matching and Categorization**
		fc.assert(
			fc.property(fc.constantFrom(...HARD_SKILLS), (hardSkill) => {
	...
	it('Property 2: Known soft skills are categorized as soft_skill', () => {
		// **Feature: optimization-cockpit, Property 2: Keyword Matching and Categorization**
		fc.assert(
			fc.property(fc.constantFrom(...SOFT_SKILLS), (softSkill) => {
				...
				const found = keywords.find(
					(k) =>
						normalizeKeyword(k.keyword).includes(normalizeKeyword(softSkill)) ||
	...
	it('Property 2: Matched keywords appear in both job and resume', () => {
		...
		fc.assert(
			fc.property(
				...
				fc.array(fc.constantFrom(...HARD_SKILLS), { maxLength: 5, minLength: 1 }),
				(resume, skills) => {
	...
	it('Property 2: Keywords are sorted by category priority', () => {
		...
		fc.assert(
			fc.property(
				fc.tuple(fc.constantFrom(...HARD_SKILLS), fc.constantFrom(...SOFT_SKILLS)),
				([hardSkill, softSkill]) => {
	...
	it('Property 2: Frequency is always positive', () => {
		...
		fc.assert(
			fc.property(
				fc.array(fc.constantFrom(...HARD_SKILLS, ...SOFT_SKILLS), { maxLength: 10, minLength: 1 }),
				(skills) => {
	...
	it('Property 2: No duplicate keywords in extraction result', () => {
		...
		fc.assert(
			fc.property(
				fc.array(fc.constantFrom(...HARD_SKILLS, ...SOFT_SKILLS, ...GENERAL_KEYWORDS), {
					maxLength: 20,
					minLength: 1,
				}),
				(skills) => {
	...
	it('Property 2: categorizeKeyword is consistent with extractCategorizedKeywords', () => {
		// **Feature: optimization-cockpit, Property 2: Keyword Matching and Categorization**
		fc.assert(
			fc.property(fc.constantFrom(...HARD_SKILLS), (skill) => {
				...
				const found = extracted.find((k) => normalizeKeyword(k.keyword) === normalizeKeyword(skill))
